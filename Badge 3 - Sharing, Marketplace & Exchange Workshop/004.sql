...
AS (SELECT DISTINCT POSTAL_CODE FROM WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY WHERE POSTAL_CODE LIKE '481%' OR POSTAL_CODE LIKE '482%');

SELECT COUNT(*) FROM WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY;

SELECT COUNT(*) FROM WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY H JOIN DEMO_DB.PUBLIC.DETROIT_ZIPS D ON H.POSTAL_CODE = D.POSTAL_CODE;

SELECT max(date_valid_std), min(date_valid_std) from history_day hd 
join demo_db.public.detroit_zips dz on hd.postal_code = dz.postal_code;

SELECT max(date_valid_std), min(date_valid_std) from forecast_day fd 
join demo_db.public.detroit_zips dz on fd.postal_code = dz.postal_code;
